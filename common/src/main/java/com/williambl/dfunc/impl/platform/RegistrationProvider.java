...
public interface RegistrationProvider<T> {
     ...
     * @return the provider
     */
    static <T> RegistrationProvider<T> getOrCreate(ResourceKey<? extends Registry<T>> resourceKey, String modId) {
        return Factory.INSTANCE.getOrCreate(resourceKey, modId);
    }
     ...
     * @return the provider
     */
    static <T> RegistrationProvider<T> getOrCreate(Registry<T> registry, String modId) {
        return Factory.INSTANCE.getOrCreate(registry, modId);
    }
    ...
    interface Factory {
         ...
         */
        Factory INSTANCE = DataFunctionsServices.load(Factory.class);
         ...
         * @return the provider
         */
        <T> RegistrationProvider<T> getOrCreate(ResourceKey<? extends Registry<T>> resourceKey, String modId);
         ...
         * @return the provider
         */
        default <T> RegistrationProvider<T> getOrCreate(Registry<T> registry, String modId) {
            return getOrCreate(registry.key(), modId);
        }
    }
}